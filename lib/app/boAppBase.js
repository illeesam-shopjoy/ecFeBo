...
(function () {
  ...
  x-buyer-id: BUYER_001 | x-license-code: eyJz~ (64자 초과 시 앞 64자 + ~) */
      ...
      const _PREF_KEYS = {
        ...
        favorites:        'ui.favorites',       // 즐겨찾기 화면ID 목록 (콤마 구분)
        favoritesKeep:    'ui.favorites_keep',  // 즐겨찾기 중 "열 때 상태 유지(📌)" 화면ID 목록 (콤마 구분)
      };
      ...
      const _syncPrefsFromDb = async () => {
        ...
        try {
          ...
          /* 즐겨찾기 — DB 가 원본. 저장값이 없으면(신규 사용자/다른 계정으로 로그인) 이전 계정 값이 남지 않게 비운다 */
          if (!embed.value) { // 독립화면(새창/embed)은 좌측 즐겨찾기 UI 가 없으므로 반영하지 않는다
            favorites.splice(0, favorites.length, ..._csvToList(p[_PREF_KEYS.favorites]));
            favKeepSet.clear();
            _csvToList(p[_PREF_KEYS.favoritesKeep]).forEach((id) => favKeepSet.add(id));
          }
        } finally {
          _suppressPrefSave = false;
        }
      };
      /* 즐겨찾기 목록 <-> 콤마 구분 문자열 (화면ID 에는 콤마가 없다) */
      const _csvToList = (csv) => [...new Set(String(csv || '').split(',').map((v) => v.trim()).filter(Boolean))];
      ...
      // 2026-09-14(요청사항: "설정 아래쪽에 local, dev, prod 여부 / api url / cdn url 3줄 표시해줘")
      const envBadgeBo = Vue.computed(() => {
        const env = window.envBoConsts || {};
        const hostPort = (host, port) => (host ? host + (port ? ':' + port : '') : '(상대경로)');
        return { mode: env.runMode || 'local', api: hostPort(env.baseApiHost, env.baseApiPort), cdn: hostPort(env.cdnApiHost, env.cdnApiPort) };
      });
      ...
      /* ── 즐겨찾기 ── */
      /* 2026-09-19: 예전엔 메모리(reactive([]))에만 있어 새로고침하면 사라졌고 DB 에도 저장되지 않았다.
       * 이제 다른 개인화 설정(ui.left_menu_open 등)과 같은 경로(localStorage 선반영 + sy_user_pref DB 저장)로 영속화한다.
       * 화면 로드 직후엔 localStorage 값을 즉시 보여주고, 로그인/토큰 복구 후 _syncPrefsFromDb 가 DB 값으로 확정한다. */
      const favorites = reactive(_csvToList(localStorage.getItem('modu-bo-' + _PREF_KEYS.favorites.replace(/\./g, '-'))));
      const favKeepSet = reactive(new Set(_csvToList(localStorage.getItem('modu-bo-' + _PREF_KEYS.favoritesKeep.replace(/\./g, '-'))))); // 즐겨찾기별 keep 설정
      const _saveFavs = () => {
        _savePref(_PREF_KEYS.favorites, favorites.join(','));
        _savePref(_PREF_KEYS.favoritesKeep, [...favKeepSet].join(','));
      };
      ...
      return {
        ...
        setApiRes,
        closeApiResPanel,
        boSettingShow, apiToastEnabled, onToggleApiToast, onToggleBoSetting, envBadgeBo,
