...
(function (global) {
  ...
  const getP = (url, cfg) => coUtil.cofGetPaged(client(), url, cfg);
  ...
  coApiSvc.cmAppInfo = {
    getInfo(uiNm, cmdNm) {
      return getP('/co/cm/app-info/info', hdr(uiNm, cmdNm));
    },
  };
  ...
  coApiSvc.cmAttach = {
    ...
    getById(attachId, uiNm = '첨부파일', cmdNm = '단건조회') {
      return chkId(attachId, uiNm, cmdNm) || getP(`/co/cm/upload/attach/${attachId}`, hdr(uiNm, cmdNm));
    },
    /* getFilesByRef — 관련테이블명+관련ID 로 직접 연계된 첨부 조회 */
    getFilesByRef(refTableNm, refId, uiNm = '첨부파일', cmdNm = '목록조회') {
      return getP('/co/cm/upload/ref/files', { params: { refTableNm, refId }, ...hdr(uiNm, cmdNm) });
    },
    ...
    getRefTableOptions(uiNm = '첨부파일', cmdNm = '연계대상목록조회') {
      return getP('/co/cm/upload/ref/table-options', hdr(uiNm, cmdNm));
    },
  ...
  coApiSvc.cmVideo = {
    ...
    getInfo(videoPath, uiNm, cmdNm) {
      return getP(`/co/cm/video/info/${videoPath}`, hdr(uiNm, cmdNm));
    },
  };

  /* ── cm: 지도 키 (/co/cm/map) — 카카오/네이버 맵 공개키 발급 (FO·BO 공용) ── */
  coApiSvc.cmMap = {
    getKeys(uiNm, cmdNm) { return getP('/co/cm/map/keys', hdr(uiNm, cmdNm)); },
  };

  /* ── cm: 토스 결제 (/co/cm/toss) — 클라이언트키 조회 + 결제 승인/취소 (FO·BO 공용) ── */
  coApiSvc.cmToss = {
    getClientKey(uiNm, cmdNm)  { return getP('/co/cm/toss/client-key', hdr(uiNm, cmdNm)); },
  ...
  coApiSvc.syCode = {
    getList(params, uiNm, cmdNm) {
      return getP('/co/sy/code', { params, ...hdr(uiNm, cmdNm) });
    },
    getGrpCodes(codeGrp, uiNm, cmdNm) {
      return getP('/co/sy/code', { params: { codeGrp }, ...hdr(uiNm, cmdNm) });
    },
    ...
    getGrpsCodes(codeGrps, uiNm, cmdNm, compNm) {
      ...
      return getP('/co/sy/code/groups', { params, ...hdr(uiNm, cmdNm) });
    },
  };
  ...
  coApiSvc.sySite = {
    getSiteList(params, uiNm, cmdNm) {
      ...
      return getP('/co/sy/site', { params, ...hdr(uiNm, cmdNm) });
    },
    getSiteById(_id, uiNm, cmdNm) {
      return chkId(_id, uiNm, cmdNm) || getP(`/co/sy/site/${_id}`, hdr(uiNm, cmdNm));
    },
  };
  ...
  coApiSvc.mbMember = {
    getPage(params, uiNm, cmdNm) {
      return getP('/co/ec/mb/member/page', { params, ...hdr(uiNm, cmdNm) });
    },
  };
  ...
  coApiSvc.syUser = {
    getPage(params, uiNm, cmdNm) {
      return getP('/co/sy/user/page', { params, ...hdr(uiNm, cmdNm) });
    },
  };
