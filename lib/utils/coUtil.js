...
(function (global) {
  ...
  /* cofExportPdf — 화면의 특정 영역(DOM 엘리먼트)을 그대로 캡처해 PDF 로 저장한다. FO·BO 공용
   * (원래 boUtil.bofExportPdf 였으나 FO 에서도 필요해져 2026-08-23 coUtil 로 이동, boUtil.bofExportPdf
   * 는 BO 로그인 사용자를 채워 넘기는 얇은 래퍼로 유지).
   * 한글을 jsPDF 자체 폰트로 그리면 CJK 미지원이라 깨지므로, html2canvas 로 브라우저가 실제
   * 그린 화면을 이미지화한 뒤 그 이미지를 jsPDF 에 얹는 방식을 쓴다 — A4 폭에 맞춰 축소하고,
   * 내용이 한 페이지보다 길면 자동으로 다음 페이지에 이어 붙인다.
   * 각주(파일명/URL/생성일시/다운로드한 로그인 사용자)도 같은 이유로 jsPDF.text() 대신 숨김 DOM을
   * html2canvas 로 캡처한 이미지로 찍는다 — 파일명 등에 한글이 들어가면 pdf.text() 가 깨지기 때문.
   * @param el        캡처할 DOM 엘리먼트(ref.value)
   * @param filename  저장 파일명("xxx.pdf")
   * @param showToast 실패 시 안내용(옵션) — 화면마다 있는 그 showToast 그대로 넘기면 됨
   * @param curUser   각주에 표시할 로그인 사용자 { authId, authNm/name/email } — 없으면 이름줄 생략 */
  async function cofExportPdf(el, filename, showToast, curUser) {
    ...
    try {
      ...
      const _now = new Date();
      const generatedAt = `${_now.getFullYear()}-${_pad(_now.getMonth() + 1)}-${_pad(_now.getDate())} `
        + `${_pad(_now.getHours())}:${_pad(_now.getMinutes())}:${_pad(_now.getSeconds())}`;
      const outFilename = filename || 'export.pdf';
      ...
      /* 각주 이미지 생성 — 파일명(한글 가능)·URL·생성일시·다운로드한 로그인 사용자를 숨김 DOM에
         그려 html2canvas 로 캡처. Page N / M 은 숫자/슬래시뿐이라 jsPDF 기본 폰트로도 안 깨져서
         pdf.text() 그대로 사용. */
      const _curUser = curUser || {};
      /* 개인정보 마스킹 — 이름은 단어별 앞 2자만 노출 + 나머지 *, ID는 앞4자:::뒤6자 */
      const _maskNm = (nm) => !nm ? '-' : String(nm).split(/\s+/)
        .map((w) => w.length <= 2 ? w : w.slice(0, 2) + '*'.repeat(w.length - 2)).join(' ');
      const _maskId = (id) => {
        const s = String(id || '');
        return s.length > 10 ? (s.slice(0, 4) + ':::' + s.slice(-6)) : s;
      };
      const _curUserNm = _maskNm(_curUser.authNm || _curUser.name || _curUser.memberNm || _curUser.email);
      const userLine = (_curUser.authId) ? `이름: ${_curUserNm} #${_maskId(_curUser.authId)}` : '';
      ...
      try {
        const footerCanvas = await window.html2canvas(footerDiv, { scale: 2, backgroundColor: '#ffffff' });
      ...
      /* 페이지번호는 전체 페이지 수를 알아야 하므로 이미지 배치가 모두 끝난 뒤 각 페이지를
         다시 방문(setPage)하며 그린다 */
      const totalPages = pdf.internal.getNumberOfPages();
      const footerBottomInset = 3; /* 페이지 맨 아래에 최대한 가깝게 */
      const footerY = 297 - footerBottomInset - footerImgH;
      for (let p = 1; p <= totalPages; p++) {
        pdf.setPage(p);
        /* 워터마크(ShopJoy, 대각선, 아주 흐리게) — 페이지 4곳에 타일링. GState 미지원 환경이면 조용히 생략 */
        try {
          pdf.saveGraphicsState();
          pdf.setGState(new pdf.GState({ opacity: 0.018 }));
          pdf.setFontSize(64);
          pdf.setTextColor(120, 120, 120);
          const wmPositions = [[60, 90], [150, 90], [60, 210], [150, 210]];
          wmPositions.forEach(([wx, wy]) => {
            pdf.text('ShopJoy', wx, wy, { angle: 45, align: 'center' });
          });
          pdf.restoreGraphicsState();
        } catch (e) { /* GState 미지원 시 워터마크 생략, 본문/각주는 정상 출력 */ }
      ...
      console.error('[coUtil.cofExportPdf]', e);
  ...
  /* cofGetPaged — GET 요청의 pageNo/pageSize 를 URL 쿼리(/page?pageNo=1&pageSize=20)로 붙여 client.get 호출. (2026-09-19 요청)
   *   boApiSvc / coApiSvc / foApiSvc 의 모든 GET 이 이 함수를 거친다 (axios 인터셉터가 아닌 svc 단에서 처리).
   *   · cfg.params 에 pageNo/pageSize 가 실제로 있을 때만 적용 — 없는데 빈 키를 만들지 않는다
   *   · 나머지 파라미터는 종전대로 cfg.params 에 둔다 (axios 는 url 에 '?' 가 있으면 params 를 '&' 로 이어 붙임 → 서버가 받는 값은 동일)
   *   · cfg.params 는 화면 상태 객체일 수 있어 원본은 건드리지 않고 복사본에서 두 키를 뺀다 */
  function cofGetPaged(client, url, cfg) {
    var pp = cfg && cfg.params;
    if (!pp || typeof pp !== 'object' || (typeof URLSearchParams !== 'undefined' && pp instanceof URLSearchParams)
        || (pp.pageNo == null && pp.pageSize == null)) {
      return client.get(url, cfg);
    }
    var q = [];
    ['pageNo', 'pageSize'].forEach(function (k) { if (pp[k] != null) q.push(k + '=' + encodeURIComponent(pp[k])); });
    var rest = {};
    Object.keys(pp).forEach(function (k) { if (k !== 'pageNo' && k !== 'pageSize') rest[k] = pp[k]; });
    return client.get(url + (url.indexOf('?') === -1 ? '?' : '&') + q.join('&'), Object.assign({}, cfg, { params: rest }));
  }
  ...
  global.coUtil.cofExportPdf = global.coUtil.cofExportPdf || cofExportPdf;
  ...
  global.coUtil.cofGetPaged = global.coUtil.cofGetPaged || cofGetPaged;
