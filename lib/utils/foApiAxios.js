...
(function (global) {
   ...
   *
   * 2026-09-14 버그수정("CORS 오류나네" — preflight redirect 차단) — envFoConsts.dev/prod의
   * baseApiHost/cdnApiHost는 "22300.illeesam.synology.me" 같은 서브도메인 라우팅 주소인데,
   * 이 NAS의 리버스프록시는 이 서브도메인들에 한해 HTTP→HTTPS 리다이렉트를 강제한다. 페이지
   * 자체가 HTTP로 떠 있으면(예: 테스트용 ecBeGateway, :22099, TLS 미적용) 위 규칙대로
   * location.protocol(=http:)을 그대로 썼다가 이 호스트로 http 요청 → 302 리다이렉트가 붙고,
   * 브라우저는 CORS preflight(OPTIONS) 요청에 대해서는 리다이렉트를 절대 따라가지 않아
   * "Redirect is not allowed for a preflight request"로 막힌다. 서브도메인 라우팅 호스트는
   * 지금 페이지 프로토콜과 무관하게 항상 https로 강제한다(포트 기반 직접 접근 호스트/로컬
   * 호스트명은 원래대로 location.protocol 그대로 따름). */
