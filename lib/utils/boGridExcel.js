...
(function (global) {
  ...
  function isControlOnly(td) {
    ...
    /* 값을 담는 컨트롤(텍스트/숫자/날짜 입력, 선택상자, 텍스트영역)이 있으면 "데이터 열" — 행 액션 열이 아니다.
       (BoGridCrud 의 인라인 편집 셀은 글자 없이 input 값만 있어서, 이걸 빼먹으면 편집 열이 통째로 사라진다) */
    if (td.querySelector('select,textarea,input:not([type=checkbox]):not([type=radio]):not([type=button]):not([type=submit]):not([type=hidden])')) return false;
