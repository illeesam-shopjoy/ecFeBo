...
window.PmPlanDtl = {
  ...
  setup(props) {
    ...
    const uiState = reactive({ loading: false, showProdPopup: false, showVendorModal: false, showTimedealProdPopup: false, showTimedealForm: false, error: null, tab: window._ecPlanDtlState.tab || 'info', tabMode2: window._ecPlanDtlState.tabMode || 'tab', activeContentTab: 1, prodSearch: ''});
    const nextId = window.nextId || { value: (arr, key) => ((arr || []).reduce((mm, x) => Math.max(mm, Number(x?.[key]) || 0), 0) || 0) + 1 };
    const timedealItems = reactive([]);
    const timedealSkus = reactive([]);
    const timedealForm = reactive({ planItemId: '', prodId: '', prodSkuId: '', dealPrice: null, totalQty: null, startDate: '', endDate: '' });
    ...
    const handleSelectAction = (cmd, param = {}) => {
      ...
      // 타임딜 미전환 대상상품 제거
        ...
        return toggleTimedealProduct(param.prodId);
      } else {
    ...
    const fnCallbackModal = (popCmd, param, result) => {
      console.log(' ■■ PmPlanDtl : fnCallbackModal -> ', popCmd, param, result);
    ...
    /* 타임딜 대상상품 (pm_plan_item + rs_pool) */

    /* loadTimedealItems — 기획전에 연결된 대상상품(+타임딜 정보) 목록 로드 */
    const loadTimedealItems = async () => {
      if (cfIsNew.value) { timedealItems.splice(0, timedealItems.length); return; }
      try {
        const res = await boApiSvc.pmPlanItem.getList({ planId: cfCurId.value }, '기획전관리', '타임딜목록조회');
        timedealItems.splice(0, timedealItems.length, ...(res.data?.data || []));
      } catch (e) { console.warn('[PmPlanDtl.js] timedeal items load failed', e); }
    };

    /* cfTimedealRows — 상품명 조인(products 목록에서 prodId 매칭) */
    const cfTimedealRows = computed(() => timedealItems.map(it => {
      const p = products.find(x => x.productId === it.prodId || x.prodId === it.prodId);
      return { ...it, prodNm: p ? (p.prodNm || p.productNm) : it.prodId };
    }));

    /* toggleTimedealProduct — 타임딜 대상상품 추가/제거 (타임딜 미등록 상태에서만 제거 가능) */
    const toggleTimedealProduct = async (prodId) => {
      const existing = timedealItems.find(it => it.prodId === prodId);
      if (existing) {
        if (existing.dealPoolId) { showToast('타임딜이 등록된 상품은 먼저 타임딜을 취소해주세요.', 'error'); return; }
        try {
          await boApiSvc.pmPlanItem.remove(existing.planItemId, '기획전관리', '타임딜대상제거');
          await loadTimedealItems();
        } catch (err) { _afterApiErr(err); }
        return;
      }
      try {
        await boApiSvc.pmPlanItem.create({
          siteId: form.siteId, planId: cfCurId.value, prodId,
          sortOrd: nextId.value(timedealItems, 'sortOrd'),
        }, '기획전관리', '타임딜대상추가');
        await loadTimedealItems();
      } catch (err) { _afterApiErr(err); }
    };
    ...
    const openTimedealForm = async (row) => {
      Object.assign(timedealForm, {
        planItemId: row.planItemId, prodId: row.prodId, prodSkuId: '',
        dealPrice: null, totalQty: null, startDate: '', endDate: '',
      });
      timedealSkus.splice(0, timedealSkus.length);
      try {
        const res = await boApiSvc.pdProd.getSkus(row.prodId, '기획전관리', '타임딜SKU조회');
        timedealSkus.splice(0, timedealSkus.length, ...(res.data?.data || []));
      } catch (e) { console.warn('[PmPlanDtl.js] sku load failed', e); }
    ...
    const submitTimedealForm = async () => {
      ...
      try {
        await boApiSvc.pmPlanItem.createTimedeal(cfCurId.value, timedealForm.planItemId, {
    ...
    const cancelTimedealItem = async (row) => {
      const ok = await showConfirm('타임딜 취소', `[${row.prodNm || row.prodId}] 타임딜을 취소하시겠습니까? (이미 판매된 수량은 유지됩니다)`);
      if (!ok) { return; }
      try {
        await boApiSvc.pmPlanItem.cancelTimedeal(row.planItemId, '기획전관리', '타임딜취소');
    ...
    const handleDelete = async () => {
      if (cfIsNew.value || !form.planId) { return; }
      const ok = await showConfirm('삭제', `[${form.planNm}]을 삭제하시겠습니까?`);
      if (!ok) { return; }
      try {
        await boApiSvc.pmPlan.remove(form.planId, '기획전관리', '삭제');
        showToast('삭제되었습니다.', 'success');
        props.navigate('pmPlanMng', { reload: true });
      } catch (err) {
        console.error('[catch-info]', err);
        const errMsg = (err.response?.data?.message) || err.message || '오류가 발생했습니다.';
        if (showToast) { showToast(errMsg, 'error', 0); }
    ...
    return {
      ...
      cfIsNew, cfSaveDisabled, cfDtlMode, cfSelectedProducts, cfSelectedVendorNm, cfTimedealRows,                          // computed
